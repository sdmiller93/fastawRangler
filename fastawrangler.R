...
# read in fasta file with spaces replaced with ;
fasta <- read.delim(file = "fasta_nospaces.fasta", sep = "\t", as.is = TRUE, header = FALSE)
		
			
# find the full header name given the list of ids
for (i in 1:length(names_final)){
			
	tempidd <- names_final[i]
	full_ids[i] <- fasta[which(grepl(tempidd, fasta)),]
...
if (keepseqs == TRUE){
	...
	newfasta <- fasta[names(fasta) %in% full_ids]
